...
const corsOptions = {
  origin: [
    "http://localhost:3000",
    "http://27.0.234.33:3000",
    "http://95.217.42.233:3000",
...
app.post("/api/auth/signup", async (req, res) => {
  try {
    ...
    if (!email) {
      return res.status(201).json({ message: "メールアドレスは必須です。" });
    }
    ...
    if (existingUser) {
      return res
        .status(201)
        .json({ message: "メールがすでに登録されています。" });
    }

    const now = new Date();
    // const name = email.split("@")[1] + '_' + 
    //     now.getFullYear().toString().slice(-2) + // Get last 2 digits of year
    //     String(now.getMonth() + 1).padStart(2, '0') + // Month (add 1 as it's 0-based)
    //     String(now.getDate()).padStart(2, '0');
    // console.log(name);
    const name = email;
    const password = generateContractId();
    const newClient = await prisma.client.create({
      data: {
        contractId: password, // Save the generated contract ID
      },
    });
    const hashedPassword = await bcrypt.hash(password, 10);

    const transporter = nodemailer.createTransport({
      host: process.env.EMAIL_HOST, // Your mail host
      port: process.env.EMAIL_PORT, // SMTP port for TLS (use 465 for SSL)
    ...
    await transporter.sendMail({
      ...
      to: email,
      subject: "新規アカウント登録のお知らせ",
      html: `<p>ご登録ありがとうございます！</p>
       <p>アカウントが正常に作成されました。</p>
       <p>以下のリンクからログインできます：</p>
       <p><a href="${process.env.SERVER_URL}/auth/signin">ログイン</a></p>
       <p>メールアドレス：${email}</p>
    ...
    const user = await prisma.user.create({
      data: {
        ...
        contractId: newClient.contractId,
      },
    });
    ...
    return res.status(200).json({
      message: "ユーザーが正常に作成されました。ログインしてください。",
      token,
    });
  } catch (error) {
    console.error(error);
...
app.post("/api/auth/signin", async (req, res) => {
  try {
    ...
    if (!user) {
      return res
        .status(400)
        .json({ message: "メールアドレスやパスワードが正しくありません。." });
    }
    ...
    return res.status(200).json({ message: "ログインに成功しました。", token, user });
  } catch (error) {
    console.error("Error during login:", error);
    return res.status(500).json({ message: "サーバー エラーです。" });
  }
});
app.post("/api/auth/phoneadd", async (req, res) => {
  const { phone, email } = req.body;
  const user = await prisma.user.findUnique({ where: { email } });
  if (!user) {
    return res.status(400).json({ message: "新規登録申請はありません。サインアップしてもう一度お試しください。" });
  }
  const findUser = await prisma.user.findMany({ where: { phone } });
  if (findUser.length > 0) {
    return res.status(400).json({ message: "電話番号がすでに存在しています。" });
  }
  const updateUser = await prisma.user.update({
    where: { id: user.id },
    data: { phone },
  });
  return res.status(200).json({ message: "電話番号を登録しました。", updateUser });
});
...
`);
    ...
    if (
      error.code === "P2002" &&
      error.meta?.target?.includes("requestRandId")
    ) {
      return res
        .status(409)
        .json({ error: "Duplicate requestRandId. Please try again." });
    }
...
app.post("/api/requestLists_mana", async (req, res) => {
  ...
  try {
    ...
    if (user.role === 0) {
      return res
        .status(301)
        .json({ error: "This is allowed only for a manager" });
    }
    ...
    combinedRequests.forEach((request) => {
      ...
      Object.keys(tp_areaSelection).forEach((areaKey) => {
        tp_areaSelection[areaKey].forEach((area) => {
          Object.keys(tp_workSelection).forEach((categoryKey) => {
            tp_workSelection[categoryKey].forEach((item) => {
              const itemFound = redStoreItems.find(
                (redItem) =>
                  redItem.category === item &&
                  redItem.address.includes(area.trim())
              );
    ...
    for (const record of records) {
      ...
      const existingItem = await prisma.RedStoreItem.findFirst({
        where: {
          ...
          address: `${mappedRecord["prefecture"] ?? ""} ${
            mappedRecord["city"] ?? ""
          } ${mappedRecord["address"] ?? ""}`,
        }, // Assuming 'url' is the unique column
      });
      if (existingItem) {
        return res
          .status(201)
          .json({ message: "データはすでに保存されています。" });
      }
      const checkValueInOptions = (data, value) =>
        data.some((item) => item.options.includes(value));

      if (
        !(
          checkValueInOptions(
            requestGroupCheckData3,
            mappedRecord["prefecture"]
          ) &&
          checkValueInOptions(requestGroupCheckData5, mappedRecord["category"])
        )
      ) {
